...
import com.babelsoftware.loudly.db.InternalDatabase
...
import com.babelsoftware.loudly.extensions.div
import com.babelsoftware.loudly.extensions.tryOrNull
import com.babelsoftware.loudly.extensions.zipInputStream
import com.babelsoftware.loudly.extensions.zipOutputStream
import com.babelsoftware.loudly.playback.MusicService
import com.babelsoftware.loudly.R
import com.babelsoftware.loudly.ui.utils.compareSong
...
class BackupRestoreViewModel @Inject constructor(
            ...
            context.applicationContext.contentResolver.openOutputStream(uri)?.use {
                it.buffered().zipOutputStream().use { outputStream ->
                    ...
                    FileInputStream(database.openHelper.writableDatabase.path).use { inputStream ->
                        outputStream.putNextEntry(ZipEntry(InternalDatabase.Companion.DB_NAME))
                        inputStream.copyTo(outputStream)
                    }
                }
            }
    ...
    fun restore(context: Context, uri: Uri) {
        runCatching {
                ...
                it.zipInputStream().use { inputStream ->
                    ...
                    while (entry != null) {
                        when (entry.name) {
                            ...
                            InternalDatabase.Companion.DB_NAME -> {
                                runBlocking(Dispatchers.IO) {
                                    database.checkpoint()
                                }
                                database.close()
            ...
            context.filesDir.resolve(MusicService.Companion.PERSISTENT_QUEUE_FILE).delete()
