...
import com.babelsoftware.loudly.constants.MediaSessionConstants
import com.babelsoftware.loudly.constants.SongSortType
import com.babelsoftware.loudly.db.MusicDatabase
import com.babelsoftware.loudly.db.entities.PlaylistEntity
        ...
        LibraryResult.ofItemList(
            when (parentId) {
                ...
                MusicService.PLAYLIST -> {
                    ...
                    listOf(
                        browsableMediaItem("${MusicService.PLAYLIST}/${PlaylistEntity.Companion.LIKED_PLAYLIST_ID}", context.getString(R.string.liked_songs), context.resources.getQuantityString(R.plurals.n_song, likedSongCount, likedSongCount), drawableUri(R.drawable.favorite), MediaMetadata.MEDIA_TYPE_PLAYLIST),
                        browsableMediaItem("${MusicService.PLAYLIST}/${PlaylistEntity.Companion.DOWNLOADED_PLAYLIST_ID}", context.getString(R.string.downloaded_songs), context.resources.getQuantityString(R.plurals.n_song, downloadedSongCount, downloadedSongCount), drawableUri(R.drawable.download), MediaMetadata.MEDIA_TYPE_PLAYLIST)
                    ...
                    parentId.startsWith("${MusicService.PLAYLIST}/") ->
                        when (val playlistId = parentId.removePrefix("${MusicService.PLAYLIST}/")) {
                            PlaylistEntity.Companion.LIKED_PLAYLIST_ID -> database.likedSongs(
                                SongSortType.CREATE_DATE, true)
                            PlaylistEntity.Companion.DOWNLOADED_PLAYLIST_ID -> {
        ...
        when (path.firstOrNull()) {
            ...
            MusicService.PLAYLIST -> {
                ...
                val songs = when (playlistId) {
                    PlaylistEntity.Companion.LIKED_PLAYLIST_ID -> database.likedSongs(SongSortType.CREATE_DATE, descending = true)
                    PlaylistEntity.Companion.DOWNLOADED_PLAYLIST_ID -> {
                        val downloads = downloadUtil.downloads.value
                        database.allSongs()
                            .flowOn(Dispatchers.IO)
