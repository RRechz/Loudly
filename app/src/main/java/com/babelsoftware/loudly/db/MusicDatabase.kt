...
import com.babelsoftware.loudly.db.entities.PlaylistSongMapPreview
...
import com.babelsoftware.loudly.db.entities.SortedSongAlbumMap
import com.babelsoftware.loudly.db.entities.SortedSongArtistMap
    ...
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.query("SELECT * FROM artist".toSQLiteQuery()).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                val newId = ArtistEntity.Companion.generateArtistId()
        ...
        database.query("SELECT * FROM playlist".toSQLiteQuery()).use { cursor ->
            while (cursor.moveToNext()) {
                val oldId = cursor.getInt(0)
                val newId = PlaylistEntity.Companion.generatePlaylistId()
