...
class YouTubeTest {
    ...
    private lateinit var testKtorClient: HttpClient

    @Before
    fun setUp() {
        testKtorClient = HttpClient(OkHttp)
    }

    @After
    fun tearDown() {
        if (::testKtorClient.isInitialized) {
            testKtorClient.close()
        }
    }
    ...
    fun `Check playable stream`() = runBlocking {
        VIDEO_IDS.forEach { videoId ->
            ...
            val url = format.url!!
            println(url)
            val response = testKtorClient.get(url) {
                headers {
