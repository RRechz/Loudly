package com.babelsoftware.innertube

import com.babelsoftware.innertube.models.AccountInfo
...
import com.babelsoftware.innertube.models.GridRenderer
...
import com.babelsoftware.innertube.models.SearchSuggestions
...
import com.babelsoftware.innertube.models.getContinuation
import com.babelsoftware.innertube.models.getItems
import com.babelsoftware.innertube.models.oddElements
import com.babelsoftware.innertube.models.response.AccountMenuResponse
import com.babelsoftware.innertube.models.response.AddItemYouTubePlaylistResponse
import com.babelsoftware.innertube.models.response.BrowseResponse
import com.babelsoftware.innertube.models.response.CreatePlaylistResponse
import com.babelsoftware.innertube.models.response.GetQueueResponse
import com.babelsoftware.innertube.models.response.GetSearchSuggestionsResponse
import com.babelsoftware.innertube.models.response.GetTranscriptResponse
import com.babelsoftware.innertube.models.response.NextResponse
import com.babelsoftware.innertube.models.response.PlayerResponse
import com.babelsoftware.innertube.models.response.SearchResponse
import com.babelsoftware.innertube.pages.AlbumPage
import com.babelsoftware.innertube.pages.ArtistItemsContinuationPage
import com.babelsoftware.innertube.pages.ArtistItemsPage
import com.babelsoftware.innertube.pages.ArtistPage
import com.babelsoftware.innertube.pages.BrowseResult
import com.babelsoftware.innertube.pages.ExplorePage
import com.babelsoftware.innertube.pages.HistoryPage
import com.babelsoftware.innertube.pages.HomePage
import com.babelsoftware.innertube.pages.LibraryContinuationPage
import com.babelsoftware.innertube.pages.LibraryPage
import com.babelsoftware.innertube.pages.MoodAndGenres
import com.babelsoftware.innertube.pages.NewReleaseAlbumPage
import com.babelsoftware.innertube.pages.NextPage
import com.babelsoftware.innertube.pages.NextResult
import com.babelsoftware.innertube.pages.PlaylistContinuationPage
import com.babelsoftware.innertube.pages.PlaylistPage
import com.babelsoftware.innertube.pages.RelatedPage
import com.babelsoftware.innertube.pages.SearchPage
import com.babelsoftware.innertube.pages.SearchResult
import com.babelsoftware.innertube.pages.SearchSuggestionPage
import com.babelsoftware.innertube.pages.SearchSummary
import com.babelsoftware.innertube.pages.SearchSummaryPage
...
object YouTube {
    ...
    suspend fun searchSuggestions(query: String): Result<SearchSuggestions> = runCatching {
        val response = innerTube.getSearchSuggestions(YouTubeClient.Companion.WEB_REMIX, query).body<GetSearchSuggestionsResponse>()
        SearchSuggestions(
    ...
    suspend fun searchSummary(query: String): Result<SearchSummaryPage> = runCatching {
        val response = innerTube.search(YouTubeClient.Companion.WEB_REMIX, query).body<SearchResponse>()
        SearchSummaryPage(
            summaries = response.contents?.tabbedSearchResultsRenderer?.tabs?.firstOrNull()?.tabRenderer?.content?.sectionListRenderer?.contents?.mapNotNull { it ->
                if (it.musicCardShelfRenderer != null)
                    SearchSummary(
                        ...
                        items = listOfNotNull(
                            SearchSummaryPage.Companion.fromMusicCardShelfRenderer(
                    ...
                    SearchSummary(
                        ...
                        items = it.musicShelfRenderer.contents?.getItems()
                            ?.mapNotNull {
                                SearchSummaryPage.Companion.fromMusicResponsiveListItemRenderer(it)
                            }
                            ?.distinctBy { it.id }
                            ?.ifEmpty { null } ?: return@mapNotNull null
                    )
    ...
    suspend fun search(query: String, filter: SearchFilter): Result<SearchResult> = runCatching {
        val response = innerTube.search(YouTubeClient.Companion.WEB_REMIX, query, filter.value).body<SearchResponse>()
        SearchResult(
    ...
    suspend fun searchContinuation(continuation: String): Result<SearchResult> = runCatching {
        val response = innerTube.search(YouTubeClient.Companion.WEB_REMIX, continuation = continuation).body<SearchResponse>()
        SearchResult(
    ...
    suspend fun album(browseId: String, withSongs: Boolean = true): Result<AlbumPage> = runCatching {
        val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId).body<BrowseResponse>()
    ...
    suspend fun albumSongs(playlistId: String): Result<List<SongItem>> = runCatching {
        var response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, "VL$playlistId").body<BrowseResponse>()
        val songs = response.contents?.twoColumnBrowseResultsRenderer
            ...
            ?.mapNotNull {
                it.musicResponsiveListItemRenderer?.let { it1 ->
                    AlbumPage.Companion.fromMusicResponsiveListItemRenderer(
                        it1
                    )
                }
            }!!
        ...
        while (continuation != null) {
            response = innerTube.browse(
                client = YouTubeClient.Companion.WEB_REMIX,
...
val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId).body<BrowseResponse>()
        ArtistPage(
            ...
            sections = response.contents?.singleColumnBrowseResultsRenderer?.tabs?.firstOrNull()
                ...
                ?.mapNotNull(ArtistPage.Companion::fromSectionListRendererContent)!!,
...
val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, endpoint.browseId, endpoint.params).body<BrowseResponse>()
        ...
        if (gridRenderer != null) {
            ArtistItemsPage(
                ...
                items = gridRenderer.items.mapNotNull {
                    it.musicTwoRowItemRenderer?.let { renderer ->
                        ArtistItemsPage.Companion.fromMusicTwoRowItemRenderer(renderer)
                    }
                },
            ...
            ArtistItemsPage(
                ...
                items = musicPlaylistShelfRenderer?.contents?.getItems()?.mapNotNull {
                    ArtistItemsPage.Companion.fromMusicResponsiveListItemRenderer(it)
                }!!,
...
val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, continuation = continuation).body<BrowseResponse>()
        when {
    ...
    suspend fun playlist(playlistId: String): Result<PlaylistPage> = runCatching {
        ...
        PlaylistPage(
            ...
            songs = response.contents?.twoColumnBrowseResultsRenderer?.secondaryContents?.sectionListRenderer
                ...
                ?.mapNotNull {
                    PlaylistPage.Companion.fromMusicResponsiveListItemRenderer(it)
                }!!,
    ...
    suspend fun addPlaylistToPlaylist(playlistId: String, addPlaylistId: String) = runCatching {
        innerTube.addPlaylistToPlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId, addPlaylistId)
    }

    suspend fun home(params: String? = null): Result<HomePage> = runCatching {
        var response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId = "FEmusic_home", params = params).body<BrowseResponse>()
        ...
        while (continuation != null) {
            response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, continuation = continuation).body<BrowseResponse>()
    ...
    suspend fun explore(): Result<ExplorePage> = runCatching {
        val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId = "FEmusic_explore").body<BrowseResponse>()
        ExplorePage(
    ...
    suspend fun newReleaseAlbums(): Result<List<AlbumItem>> = runCatching {
        val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId = "FEmusic_new_releases_albums").body<BrowseResponse>()
    ...
    suspend fun moodAndGenres(): Result<List<MoodAndGenres>> = runCatching {
        val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId = "FEmusic_moods_and_genres").body<BrowseResponse>()
    ...
    suspend fun browse(
        ...
        runCatching {
            val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, browseId = browseId, params = params)
                .body<BrowseResponse>()
            BrowseResult(
                title =
                    response.header
                                        ...
                                        items =
                                            content.musicCarouselShelfRenderer.contents
                                                .mapNotNull { content2 ->
                                                    ...
                                                    renderer?.let {
                                                        when (renderer) {
                                                            is MusicTwoRowItemRenderer -> RelatedPage.Companion.fromMusicTwoRowItemRenderer(
                                                                renderer
                                                            )

                                                            is MusicResponsiveListItemRenderer ->
                                                                SearchSummaryPage.Companion.fromMusicResponsiveListItemRenderer(
                                                                    renderer,
                                                                )
            ...
            contents?.gridRenderer != null -> {
                LibraryPage(
                    items = contents.gridRenderer.items
                        .mapNotNull(GridRenderer.Item::musicTwoRowItemRenderer)
                        .mapNotNull { LibraryPage.Companion.fromMusicTwoRowItemRenderer(it) },
                ...
                LibraryPage(
                    items = contents?.musicShelfRenderer?.contents!!
                        ...
                        .mapNotNull { LibraryPage.Companion.fromMusicResponsiveListItemRenderer(it) },
    ...
    suspend fun likedPlaylists(): Result<List<PlaylistItem>> = runCatching {
        ...
        val playlists = gridRenderer?.items!!
            ...
            .mapNotNull(GridRenderer.Item::musicTwoRowItemRenderer)
            .mapNotNull {
                ArtistItemsPage.Companion.fromMusicTwoRowItemRenderer(it) as? PlaylistItem
            }.toMutableList()
    ...
    suspend fun subscribeChannel(channelId: String, subscribe: Boolean) = runCatching {
        if (subscribe)
            innerTube.subscribeChannel(YouTubeClient.Companion.WEB_REMIX, channelId)
        else
            innerTube.unsubscribeChannel(YouTubeClient.Companion.WEB_REMIX, channelId)
    }
   ...
   fun createPlaylist(title: String) = runBlocking {
        innerTube.createPlaylist(YouTubeClient.Companion.WEB_REMIX, title).body<CreatePlaylistResponse>().playlistId
    }

    suspend fun likeVideo(videoId: String, like: Boolean) = runCatching {
        if (like)
            innerTube.likeVideo(YouTubeClient.Companion.WEB_REMIX, videoId)
        else
            innerTube.unlikeVideo(YouTubeClient.Companion.WEB_REMIX, videoId)
    }

    suspend fun likePlaylist(playlistId: String, like: Boolean) = runCatching {
        if (like)
            innerTube.likePlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId)
        else
            innerTube.unlikePlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId)
    }
            ...
            sections = response.contents?.singleColumnBrowseResultsRenderer?.tabs?.firstOrNull()
                ...
                ?.mapNotNull {
                    it.musicShelfRenderer?.let { musicShelfRenderer ->
                        HistoryPage.Companion.fromMusicShelfRenderer(musicShelfRenderer)
                    }
                }
        )
    }
    ...
    suspend fun libraryRecentActivity(): Result<LibraryPage> = runCatching {
        ...
        val items = response.continuationContents?.sectionListContinuation?.contents?.firstOrNull()
            ?.gridRenderer?.items!!.mapNotNull {
                it.musicTwoRowItemRenderer?.let { renderer ->
                    LibraryPage.Companion.fromMusicTwoRowItemRenderer(renderer)
                }
            }.toMutableList()
    ...
    suspend fun next(endpoint: WatchEndpoint, continuation: String? = null): Result<NextResult> = runCatching {
        val response = innerTube.next(YouTubeClient.Companion.WEB_REMIX, endpoint.videoId, endpoint.playlistId, endpoint.playlistSetVideoId, endpoint.index, endpoint.params, continuation).body<NextResponse>()
    ...
    suspend fun related(endpoint: BrowseEndpoint): Result<RelatedPage> = runCatching {
        val response = innerTube.browse(YouTubeClient.Companion.WEB_REMIX, endpoint.browseId).body<BrowseResponse>()
            ...
            sectionContent.musicCarouselShelfRenderer?.contents?.forEach { content ->
                when (val item = content.musicResponsiveListItemRenderer?.let(RelatedPage.Companion::fromMusicResponsiveListItemRenderer)
                    ...
                    is SongItem -> if (content.musicResponsiveListItemRenderer?.overlay
                            ...
                            ?.watchEndpointMusicConfig?.musicVideoType == WatchEndpoint.WatchEndpointMusicSupportedConfigs.WatchEndpointMusicConfig.Companion.MUSIC_VIDEO_TYPE_ATV
                    ) songs.add(item)
    ...
    suspend fun queue(videoIds: List<String>? = null, playlistId: String? = null): Result<List<SongItem>> = runCatching {
        ...
        innerTube.getQueue(YouTubeClient.Companion.WEB_REMIX, videoIds, playlistId).body<GetQueueResponse>().queueDatas
            .mapNotNull {
    ...
    suspend fun moveSongPlaylist(playlistId: String, setVideoId: String, successorSetVideoId: String) = runCatching {
        innerTube.moveSongPlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId, setVideoId, successorSetVideoId)
    }

    suspend fun renamePlaylist(playlistId: String, name: String) = runCatching {
        innerTube.renamePlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId, name)
    }

    suspend fun deletePlaylist(playlistId: String) = runCatching {
        innerTube.deletePlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId)
    }

    suspend fun transcript(videoId: String): Result<String> = runCatching {
        val response = innerTube.getTranscript(YouTubeClient.Companion.WEB, videoId).body<GetTranscriptResponse>()
    ...
    suspend fun accountInfo(): Result<AccountInfo> = runCatching {
        innerTube.accountMenu(YouTubeClient.Companion.WEB_REMIX).body<AccountMenuResponse>()
    ...
    suspend fun addToPlaylist(playlistId: String, videoId: String) = runCatching {
        innerTube.addToPlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId, videoId).body<AddItemYouTubePlaylistResponse>()
    }
    suspend fun removeFromPlaylist(playlistId: String, videoId: String, setVideoId: String) = runCatching {
        innerTube.removeFromPlaylist(YouTubeClient.Companion.WEB_REMIX, playlistId, videoId, setVideoId)
    }
    @JvmInline
